...
const T: Record<BridgeLang, Tr> = {
  de: {
    ...
    anycubicHint: 'Am Drucker-Display den LAN-Modus einschalten und anlassen (Einstellungen → Netzwerk). Die IP steht dort ebenfalls. Kein Passwort nötig — die Bridge holt die Zugangsdaten automatisch vom Drucker. Drucken vom PC geht weiter direkt im Netzwerk (Anycubic Slicer Next → „LAN Printer"); nur Handy-App und Cloud-Fernzugriff sind bei aktivem LAN-Modus aus.',
  ...
  en: {
    ...
    anycubicHint: 'Enable LAN mode on the printer display and keep it on (Settings → Network). You\'ll find the IP there too. No password needed — the bridge fetches the credentials from the printer automatically. Printing from your PC keeps working directly over the network (Anycubic Slicer Next → "LAN Printer"); only the mobile app and cloud remote access are off while LAN mode is on.',
